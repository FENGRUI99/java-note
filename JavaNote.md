...
- `bgsave`，fork一个子进程，通过**<font color='red'>`写时复制`</font>**的机制，在不影响主进程的情况下，生成二进制快照
  ...
  - 只有当父进程执行写操作时，才会真正将父进程数据拷贝到独立的物理内存中。
...
- 优点
  ...
  - 占用内存很小，一亿条数据，误判率千分之一，仅占用171M左右内存
...
![Sentinel哨兵模式](./imgs/Sentinel哨兵模式.png)
...
![Cluster集群模式](./imgs/Cluster集群模式.png)
...
![为什么Redis常规架构不适合海量请求](./imgs/为什么Redis常规架构不适合海量请求.png)
...
- 解决方案
  - 分布式锁 + 双重判断（在分布式锁的前后都去判断缓存中是否有数据）
    ...
    - 后续请求获取锁后，再去判断一下缓存中是否存在该热点数据，避免大量请求访问数据库资。
  - 缓存预热：提前将已知的热点数据写入缓存
...
- 事务隔离级别
  ...
  | 读已提交 | 允许读取已提交的修改数据               | 幻读，不可重复读                                | MVCC，锁 |
...
- select for update读取数据时对数据上锁，方便**同一个事务**内其他操作对数据修改，适用于数据严格一致性场景，比如银行转账、库存管理
...
![深分页问题](imgs/深分页问题.png)

- 我们可以使用上次查询的最大值来当作这次的查询条件（游标分页）

### 11.有一条SQL执行慢，怎么排查

![优化慢SQL](imgs/优化慢SQL.png)
...
### 14. 执行一条 select 语句的过程

![一条SQL执行的过程](imgs/一条SQL执行的过程.png)

### 15.从数据页的角度看索引

![从数据页的角度看索引](imgs/从数据页的角度看索引.png)
...
- 柔性事务
  ...
  - MQ事务
    ...
    - 半消息回查：由于某些原因导致生产者没有发送半消息的二次确认，MQ主动向生产者发消息确认事务状态
    - 流程
      1. 生产者向MQ发送半消息
      2. MQ持久化半消息，回复生产者ACK
      3. 生产者执行本地事务
...
![为什么用消息队列](imgs/为什么用消息队列.png)

- 消息队列一般有三种用处：削峰、异步、解耦
- **削峰/限流**
...
![为什么是RocketMQ](imgs/为什么是RocketMQ.png)
...
![为什么RocketMQ快](imgs/为什么RocketMQ快.png)
...
有点类似于接口和类的关系，方法区是接口，永久代(1.8被替代)和元空间是实现类

### Java对象创建的过程

![Java对象创建](imgs/Java对象创建.png)

- Java创建对象一般分为五个步骤
- **类加载检验**：先去验证需要创建的对象对应的这个类是否被加载，如果没有被加载，执行类加载流程
- **分配内存**：给对象分配内存，一般有两种方式——指针碰撞和空闲散列，具体选择需要根据堆内存是否规整来定
  - ~~指针碰撞：已用内存放一边，未用内存放一边，中间用指针隔开，分配内存时就移动指针~~
  - ~~空闲散列：维护一个表，记录可用的内存地址~~
- **初始化零值**：将分配好的内存的每一位都置为0
- **设置对象头**：对象头内记录元数据，比如哈希码
- **执行init方法**：调用构造函数，初始化对象

### 判断对象是否死亡

![判断对象死亡](imgs/判断对象死亡.png)

- 一般有两种方法，引用计数法和**可达性分析法**
- 引用计数法不常用，因为无法解决对象循环引用的问题
- 可达性分析法：根据`GC Root`对象为起点的**引用链**来标记存活对象，没有标记到的就是要回收的对象
  - GC Root一般选用虚拟机栈中的局部变量引用的对象、常量池引用的对象、静态变量引用的对象

### JDK引用类型

![引用类型](imgs/引用类型.png)

- Java中引用类型有四种，强、软、弱、虚
- 强引用：就是开发者一般new创建的对象，不会被GC回收
- 软引用：会在内存不足时回收
- 弱引用：不论内存是否充足都会被回收，`ThreadLocalMap`的`key`就是弱引用`ThreadLocal`对象
- 虚引用：与被引用的对象没有直接关联，不能通过虚引用获取到对象，虚引用用于**跟踪垃圾回收对象**

### 堆的基本结构以及对象怎么进入老年代

![对象在堆中分配](imgs/对象在堆中分配.png)

- 堆内存可以分为新生代和老年代，新生代又可以分为伊甸园区和两个幸存者区：s0和s1区
- 新创建的对象都会被分配到伊甸园区，当对象无法被分配到伊甸园区，会触发`young gc`，会将伊甸园区的对象和幸存者s0区的对象都移入s1区。如果s1区也存不下，对象会被移入老年代
- 当幸存者区的对象经历一定次数的`young gc`之后，也会进入老年代

### 垃圾回收算法

![垃圾回收算法](imgs/垃圾回收算法.png)

- 一般有三种垃圾回收算法，标记-清除算法、复制算法、标记-压缩算法
- 标记-清除算法：首先标记所有存活对象，清除所有可回收对象，会产生内存碎片
- 复制算法：
  - 会将内存分为两块，一块为使用区，一块为空闲区。垃圾回收时，只需将使用区中的存活对象复制到空闲区，然后将空闲区设置为活动区，活动区设置为空闲区
  - 在存活对象较少时，效率很高，所以一般用于新生代，因为对象存活时间短
  - 也不会产生内存碎片
  - 缺点是只能用一半内存

- 标记-压缩算法：
  - 首先标记所有存活对象，将存活对象移动至堆的一侧，按照顺序排列，其余可回收对象直接被忽略或覆盖
  - 相对性能一般，用于对象存活时间较长的老年代


### Minor GC、Full GC

- Minor GC：回收新生代，一般当伊甸园区满了触发
- Full GC：回收整个堆和方法区，当老年代内存不够用触发
- Mixed GC：G1垃圾回收器，回收新生代和部分老年代，触发条件是老年代使用率达到一定阈值

### [并发标记](https://juejin.cn/post/6844904070788939790)

![并发标记](imgs/并发标记.png)

- 并发标记可以避免用户线程长期暂停、提高吞吐量
- 并发标记通过**三色标记法**实现，将对象分为三种颜色：白、灰、黑
  - 白色对象代表对象没有被垃圾回收器访问
  - 灰色对象代表对象已被垃圾回收器访问，但是该对象至少有一个引用还没有被访问
  - 黑色对象代表对象已被垃圾回收器访问，且所有引用都被访问
- 并发标记会带来两个问题：多标和漏标
  - 多标：就是常规的浮动垃圾，原本已经标黑的对象，失去他们对该对象的引用，该对象需要被回收，但是黑色对象在这个GC不会回收
  - 漏标：有一个白色对象A，一开始被灰色对象引用，后来修改成被一个黑色对象引用，但是黑色对象不会再被访问，所以这个白色对象会被回收，造成用户拿不到想要的对象，程序出现异常
    - CMS采用**增量更新**解决漏标问题：当黑色对象引用白色对象时，**<font color='red'>将黑色对象重新设为灰色</font>**，这样会重新访问灰色对象
    - G1采用**原始快照**解决漏标问题：当灰色对象和白色对象的引用删除时，**<font color='red'>记录下这个引用</font>**，重新标记阶段，根据刚记录的引用关系，将白色对象置为黑色。因此有可能产生浮动垃圾，但是不影响，下一次GC就会被回收



### CMS垃圾回收器

<img src="imgs/CMS垃圾回收器.png" alt="CMS垃圾回收器" style="zoom:50%;" />

- CMS是一个针对老年代的垃圾回收器，它的特点是**注重系统停顿时间**，这点和注重吞吐量的parallel gc不同
- CMS同时还是第一个实现**<font color='red'>回收线程和用户线程并发存在</font>**垃圾回收的回收器，在此之前，gc在标记和回收的时候都需要暂停所有用户线程
- 工作流程是
  - 初次标记：需要STW(暂停用户线程)，标记所有与GC root直接关联的对象
  - 并发标记：同时开启回收线程和用户线程，根据root的引用链采用三色标记算法去标记对象
  - 重新标记：需要STW，对于并发标记可能尝试的漏标问题，CMS采用**增量更新**的方式解决
  - 并发清除：采用清除算法去回收对象
- 缺点
  - 因为采用标记-清除算法，会产生内存碎片
  - 无法处理**浮动垃圾**：并发清理的时候也会产生可回收对象

### G1垃圾回收器

![G1垃圾回收器](imgs/G1垃圾回收器.png)

- G1垃圾回收器与其他回收器不同的是**分区思想**，它将堆分成多个小块，使得伊甸园区、幸存者区的s0和s1区、老年代都不需要连续
- 和CMS一样，注重STW停顿时间，它建立了一个可预测的停顿时间模型，使得垃圾回收时间不会超过一个时间片
- G1对整个堆进行回收，不像CMS只针对老年代。两种常见的GC模式，young gc和Mixed GC
  - young gc触发条件就是伊甸园区满了，需要STW停顿，并行标记和复制
  - Mixed gc流程有四个阶段
    - 初始阶段：STW，标记直接与GC root相连的存活对象
    - 并发标记：同时允许回收线程和用户线程，根据GC root引用链采用三色标记算法来标记存活对象
    - 重新标记：STW，对于并发标记可能尝试的漏标问题，CMS采用**原始快照**的方式解决
    - 筛选回收：根据后台维护的优先回收列表，回收最有价值的分区（垃圾最多的分区）
...
![字节码和类文件结构](imgs/字节码和类文件结构.png)

- 字节码就是一种可以直接在JVM上运行的语言，不依赖于操作系统，可以在任何操作系统运行
- 类文件结构一般有类版本号、常量池、字段表、方法表等组成
- 常量池存的是类名和符号引用
- 字段表存的是静态变量
...
- 类加载过程主要分三步，加载、连接、初始化
- **加载**：就是通过**类加载器**，根据**双亲委派模型**加载Java类的字节码(.class文件)，在内存中生存一个该类的class对象
- **连接**具体可以分为三步
  - 验证：验证class对象的安全性
  - 准备：在方法区为静态变量分配内存
  - 解析：常量池的**符号引用**（）解析成**直接引用**（内存地址）
    - ~~符号引用：间接引用，为了独立编译和动态加载~~；~~直接引用：静态变量的内存地址~~
    - ~~个人理解，`public final static name = "cfr"`，在准备阶段给`name`分配内存，解析阶段获取`name`的直接引用（内存地址），初始化阶段赋值为"cfr"~~
- **初始化**：调用类构造器，初始化静态变量。
  - ~~触发条件：new一个该类的实例对象；调用该类的静态方法~~

### JDK默认类加载器

![类加载器](imgs/类加载器.png)

- 类加载器是用来加载Java类的字节码，在内存中生成Class对象
- 常用的类加载器有三种：
  - 启动类加载器：加载Java核心库，比如Java.lang，包括Object和所有基本数据类型
  - 扩展类加载器：加载扩展库中的类，比如java.sql，包括DataSource这种管理数据库连接的类
  - 应用加载器：加载开发者自己写的类
...
![双亲委派模型](imgs/双亲委派模型.png)

- 双亲委派模型是一种类加载机制，要求每个加载类都要有自己的父类加载器
- 工作流程：
  - 当一个类加载器收到类加载请求，先把请求委派给父类加载器
  - 父类加载器同样向上委派，直到启动类加载器
  - 如果父类加载器没有找到该类，则子类加载器执行加载任务
  - 找不到则抛出异常
- 优点：
  - **避免类重复加载**：
    - 我们知道判断一个类是否相同，不仅需要类名相同，还需要类加载器相同
    - 如果父类加载过了，子类就不必加载了，通过这样的委派机避免内存浪费
  - **保证Java核心API不被篡改**
    - 核心API都在启动类加载器或者扩展类加载器加载了，不会有机会加载盗版的API

#### 如果不用双亲委派模型

![不使用双亲委派](imgs/不使用双亲委派.png)

- 一般自定义类加载器之需要继承`ClassLoader`
- 如果遵循双亲委派的话，就改写`findClass`方法
- 如果不遵循的话，就改写`loadClass`方法

### 虚拟机故障处理⼯具

![JVM性能监控](imgs/JVM性能监控.png)
...
一级缓存单例Map（SingletonObjects） 二级缓存半成品Bean（EarlySingletonObjects） 三级缓存Bean创建工厂（SingletonFactories） 实际代码逻辑是这样： 先从一级找，没有到二级，再没有到三级，三级找到放到二级并删掉三级 AB互相依赖时 1.先创建A的工厂方法放入三级，继续属性注入 2.发现依赖属性B走B的流程，同样创建工厂方法放入三级 3.返现依赖属性A，会从三级拿到A工厂方法，将A从三级移除放入二级（放入的可能是原对象也可能是代理对象，取决于是否AOP增强）...完成B初始化流程 将B放到一级 4.继续A流程 此时B以存在，完成B属性注入... 完成A初始化流程 将A放到一级 结束
