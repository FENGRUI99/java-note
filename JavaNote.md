...
- 有界队列：有固定容量，避免线程池中的任务过载
- 无界队列：无固定容量，提高任务高峰期的吞吐量
...
- 常用队列
  ...
  - `ArrayBlockingQueue`：基于数组的&**有界阻塞队列**
    ...
    - `LinkedBlockingQueue`：基于链表的**阻塞队列**，吞吐量高于`ArrayBlockingQueue`
    ...
    - `DelayedWorkQueue`：只有当延迟时间到了，才能从队列中获取元素
...
- 解决方案
  - 分布式锁 + 双重判断锁（在分布式锁的前后都去判断缓存中是否有数据）
    - 大量请求访问一个已过期的热点数据
...
- 消息丢失场景和解决
  ...
  - **MQ持久化消息阶段**，有两种情况发生消息丢失
    ...
    - 消息写入磁盘，但是数据没有备份，磁盘损坏
      - 采用集群方式部署，主从模式
  - **消费者消费消息阶段**，还没有完成消费，就返回`ACK`给`Broker`，一旦消费失败，消息就丢失了
    - 解决方案
      - 使用手动消息确认机制，在业务代码中确保消费完成，再发送确认。
